...
public class SearchFlight {
	...
	private List<Flight> convertToLinkedListFlights(Flight[] arrayFlights){
		return new LinkedList<Flight>(Arrays.asList(arrayFlights));
	}
	 ...
	 */
	private List<ResultFlight> searchFlight(List<Flight> listFlightAux, Flight[] arrayFlights, OriginDestination[] arrayOD, OriginDestination od){
		List<ResultFlight> listResults = new ArrayList<ResultFlight>();
		int pos = 0;
		while(pos>=0){
			...
			if(pos>0){
				...
				arrayFlights = listFlightAux.toArray(new Flight[listFlightAux.size()]);
				arrayOD = OriginDestination.getFlighODs(arrayFlights);
			}
		}
		return listResults;
	}
	...
	public List<ResultFlight> searchFlightAlgorithm(String origin, String destination){
		...
		//Call the algorithm
		return searchFlight(listFlightAux, arrayFlights, arrayOD, od);
	}
